...
export async function generateContributors() {
  const contributors = await getFunctionContributors();
  for (const key of Object.keys(contributors)) {
    const contributor = contributors[key];
    contributor.sort((a: any, b: any) => {
      return b.count - a.count;
    });
  }
  const cachePath = resolve(__dirname, '../docs/public');
  const cacheFile = 'contributors.json';
  const cacheFilePath = join(cachePath, cacheFile);
  if (existsSync(cacheFilePath)) {
    rmSync(cacheFilePath);
  }
  writeFile(cacheFilePath, JSON.stringify(contributors, null, 2));
}
